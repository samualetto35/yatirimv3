                  ...
                  <div style={{ borderTop: '1px solid #f1f3f5', padding: '10px 12px' }}>
                    {pairs.length ? (
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 8 }}>
                        ...
                        {pairs.map(sym => (
                          <>
                            <div key={`${row.id}_${sym}_s`} style={{ fontWeight: 800 }}>{sym}</div>
                            <div key={`${row.id}_${sym}_o`}>${m[sym]?.open != null ? Number(m[sym].open).toFixed(2) : '—'}</div>
                            <div key={`${row.id}_${sym}_c`}>${m[sym]?.close != null ? Number(m[sym].close).toFixed(2) : '—'}</div>
                            <div key={`${row.id}_${sym}_r`} style={{ color: Number(m[sym]?.returnPct || 0) >= 0 ? '#16a34a' : '#dc2626' }}>{m[sym]?.returnPct != null ? `${Number(m[sym].returnPct).toFixed(2)}%` : '—'}</div>
                          </>
                        ))}
                      </div>
                    ) : (
